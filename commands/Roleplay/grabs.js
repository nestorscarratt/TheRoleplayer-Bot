...
module.exports = {
    name: "grabs",
    description: "Grabs or restrains a user",
    execute(message){
      ...
      if(!message.guild.me.hasPermission("MANAGE_MESSAGES"))  {
        ...
        if(member) {
        const grabs = new Discord.MessageEmbed()
        .setAuthor(`${message.author.tag}`, message.author.avatarURL({ dynamic:true }))
        .setTitle('Grabs Person')
        .setDescription(`<@${message.author.id}> Has grabbed <@${member.id}>, restraining them`)
        .setColor('GREEN');

        message.channel.send(grabs)
